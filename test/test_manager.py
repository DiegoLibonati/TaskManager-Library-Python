...
class TestManagerProperties:
    ...
    def test_tasks_keys_contains_task_id(self, manager_with_task: Manager, task: TaskModel) -> None:
        assert task.id in manager_with_task.tasks_keys

    def test_tasks_values_contains_task(self, manager_with_task: Manager, task: TaskModel) -> None:
...
class TestManagerAddTask:
    ...
    def test_add_task_stores_task_in_values(self, manager: Manager, task: TaskModel) -> None:
        manager.add_task(task=task)
        assert task in manager.tasks_values

    def test_add_task_stores_task_by_id_key(self, manager: Manager, task: TaskModel) -> None:
        manager.add_task(task=task)
        assert task.id in manager.tasks_keys

    def test_add_multiple_tasks(self, manager: Manager, expiration_date: datetime) -> None:
        task_a = TaskModel(title="A", description="A", expiration_date=expiration_date)
        task_b = TaskModel(title="B", description="B", expiration_date=expiration_date)
    ...
    def test_add_duplicate_task_error_code(self, manager: Manager, task: TaskModel) -> None:
        manager.add_task(task=task)
...
class TestManagerRemoveTask:
    ...
    def test_remove_task_not_in_values(self, manager_with_task: Manager, task: TaskModel) -> None:
        manager_with_task.remove_task(id_task=task.id)
    ...
    def test_remove_task_not_found_raises_not_found_error(self, manager: Manager, task: TaskModel) -> None:
        with pytest.raises(NotFoundError):
            manager.remove_task(id_task=task.id)

    def test_remove_task_not_found_error_code(self, manager: Manager, task: TaskModel) -> None:
...
class TestManagerEditTask:
    ...
    def test_edit_task_not_found_raises_not_found_error(self, manager: Manager, task: TaskModel) -> None:
        with pytest.raises(NotFoundError):
            manager.edit_task(id_task=task.id, title="X")

    def test_edit_task_no_properties_raises_validation_error(self, manager_with_task: Manager, task: TaskModel) -> None:
        with pytest.raises(ValidationError):
            manager_with_task.edit_task(id_task=task.id)


class TestManagerMoveTaskByState:
    def test_move_task_changes_state(self, manager_with_task: Manager, task: TaskModel) -> None:
    ...
    def test_move_task_not_found_raises_not_found_error(self, manager: Manager, task: TaskModel) -> None:
        with pytest.raises(NotFoundError):
...
class TestManagerFindTaskById:
    ...
    def test_find_missing_task_raises_not_found_error(self, manager: Manager, task: TaskModel) -> None:
        with pytest.raises(NotFoundError):
            manager._find_task_by_id(id_task=task.id)

    def test_find_missing_task_error_code(self, manager: Manager, task: TaskModel) -> None:
        with pytest.raises(NotFoundError) as exc_info:
            manager._find_task_by_id(id_task=task.id)
        assert exc_info.value.code == CODE_NOT_FOUND_TASK
