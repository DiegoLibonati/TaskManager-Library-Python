...
class TestTaskInit:
    def test_id_is_uuid(self, task: TaskModel) -> None:
    ...
    def test_title_matches(self, task: TaskModel) -> None:
        assert task.title == "Test Task"

    def test_description_matches(self, task: TaskModel) -> None:
        assert task.description == "Test Description"

    def test_expiration_date_matches(self, task: TaskModel, expiration_date: datetime) -> None:
        assert task.expiration_date == expiration_date

    def test_default_state_is_pending(self, task: TaskModel) -> None:
        assert task.state == "pending"

    def test_custom_initial_state(self, expiration_date: datetime) -> None:
        task = TaskModel(title="T", description="D", expiration_date=expiration_date, state="in_progress")
        assert task.state == "in_progress"


class TestTaskChangeState:
    def test_change_to_in_progress(self, task: TaskModel) -> None:
        task.change_state(state="in_progress")
        assert task.state == "in_progress"

    def test_change_to_complete(self, task: TaskModel) -> None:
        task.change_state(state="complete")
        assert task.state == "complete"

    def test_change_back_to_pending(self, task: TaskModel) -> None:
        task.change_state(state="in_progress")
        task.change_state(state="pending")
        assert task.state == "pending"

    def test_invalid_state_raises_validation_error(self, task: TaskModel) -> None:
        with pytest.raises(ValidationError):
            task.change_state(state="invalid_state")

    def test_invalid_state_error_code(self, task: TaskModel) -> None:
        with pytest.raises(ValidationError) as exc_info:
            task.change_state(state="invalid_state")
        assert exc_info.value.code == CODE_NOT_VALID_STATUS_TASK

    def test_empty_state_raises_validation_error(self, task: TaskModel) -> None:
        with pytest.raises(ValidationError):
            task.change_state(state="")


class TestTaskEdit:
    def test_edit_title(self, task: TaskModel) -> None:
        task.edit(title="New Title")
        assert task.title == "New Title"

    def test_edit_description(self, task: TaskModel) -> None:
        task.edit(description="New Description")
        assert task.description == "New Description"

    def test_edit_expiration_date(self, task: TaskModel) -> None:
    ...
    def test_edit_does_not_change_unset_fields(self, task: TaskModel) -> None:
        task.edit(title="Only Title Changed")
        assert task.description == "Test Description"

    def test_edit_no_properties_raises_validation_error(self, task: TaskModel) -> None:
        with pytest.raises(ValidationError):
            task.edit()

    def test_edit_no_properties_error_code(self, task: TaskModel) -> None:
        with pytest.raises(ValidationError) as exc_info:
            task.edit()
...
class TestTaskStr:
    ...
    def test_str_returns_string(self, task: TaskModel) -> None:
